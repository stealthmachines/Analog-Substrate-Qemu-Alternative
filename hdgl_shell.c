...
/* Boot config (set by hdgl_run.c) */
static boot_config_t g_boot_cfg = BOOT_CONFIG_DEFAULTS;
void shell_set_boot_config(boot_config_t *bc) { g_boot_cfg = *bc; }
...
/* legacy stub */
void shell_term_raw_UNUSED(void) {
    struct termios t;
...
void shell_term_restore_UNUSED(void) {
...
static void cmd_help(void) {
    ...
    printf("  \033[1mboot\033[0m [K] [I]     boot Alpine (kernel K, initrd I)\n");
    printf("  \033[1mkexec\033[0m K [I]      boot via kexec\n");
    printf("  \033[1mload\033[0m FILE.hdgl     load + inspect HDGL glyph file\n");
    printf("  \033[1mlattice\033[0m [TYPE]      show/select lattice type\n");
    printf("  \033[1mbootstat\033[0m          boot method status\n");
...
void shell_run(shell_config_t *cfg) {
    ...
    while (cfg->running) {
        char prompt_buf[128];
        snprintf(prompt_buf, sizeof(prompt_buf), "%sRouter64%s%s>%s ",
                 term_purple(), term_reset(), term_green(), term_reset());
        int rc = isatty(STDIN_FILENO)
               ? term_readline(prompt_buf, line, sizeof(line))
               : term_readline_simple(prompt_buf, line, sizeof(line));
        if (rc == TERM_LINE_EOF) break;
        if (rc == TERM_LINE_INT) { printf("\n"); continue; }
        if (!line[0]) continue;
        ...
        if      (!strcmp(cmd,"help"))      cmd_help();
            ...
            substrate_lock();
                    substrate_unlock();
            boot_config_t bc = g_boot_cfg;
        bc.kernel_path = a1;
        if (a2[0]) bc.initrd_path = a2;
        bc.use_live_substrate = 1;
        boot_run(&bc);
        }
